...
// T28 INGEST no-backup absence starts Monday: previous Sunday's lanes must carry forward.
// Sunday morning -> Monday morning; Sunday evening -> Monday evening. No false rest violation.
{
  const u='INGEST OPERATÖRÜ', mo='06:30–16:00', ev='16:00–00:00';
  const people=[
    {ad:'ABSENT',birim:u,izinGunleri:[]},
    {ad:'SUN_MORNING',birim:u,izinGunleri:[]},
    {ad:'SUN_EVENING',birim:u,izinGunleri:[]}
  ];
  const st=baseState(people,[u],[mo,ev]);
  st.birimAyarlari[u]={tip:'DONGU6'};
  st.mcrAyarlari={baslangicTarihi:'2026-09-14',ofsetler:{ABSENT:0,SUN_MORNING:2,SUN_EVENING:4}};
  // Önceki hafta Pazar gerçek vardiyaları: biri sabah, diğeri akşam.
  st.manuelAtamalar['2026-09-14_SUN_MORNING_6']=mo;
  st.manuelAtamalar['2026-09-14_SUN_EVENING_6']=ev;
  // ABSENT yeni haftanın tamamında yıllık izinli.
  for(let d=0;d<7;d++) st.manuelAtamalar[`2026-09-21_ABSENT_${d}`]='YILLIK İZİN';
  st.schedulerV2={manualLocks:{},assignmentSource:{},manualUnitLocks:{},tempUnitSource:{},mcrReplacements:{},config:{preferredWorkDays:5,maxWorkDays:7}};
  for(let d=0;d<7;d++) st.schedulerV2.assignmentSource[`2026-09-21_ABSENT_${d}`]='ANNUAL_LOCAL';

  const ctx=makeCtx({state:st,currentMonday:new Date('2026-09-21T12:00:00')});
  const r=ctx.SchedulerV2.solveSchedule({full:true,units:[u]});
  assert(r.ok,`T28 solve: ${(r.errors||[]).join(' | ')}`);
  assert(r.work.matrix.SUN_MORNING[0]===mo,`T28 Sunday morning must continue Monday morning, got ${r.work.matrix.SUN_MORNING[0]}`);
  assert(r.work.matrix.SUN_EVENING[0]===ev,`T28 Sunday evening must continue Monday evening, got ${r.work.matrix.SUN_EVENING[0]}`);
  console.log('T28 PASS INGEST Sunday lane carry-forward',r.work.matrix.SUN_MORNING[0],r.work.matrix.SUN_EVENING[0]);
}

console.log('FIX10 INGEST CARRY-FORWARD TESTS PASS');
